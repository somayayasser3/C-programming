...
int main()
{
	...
	print();//Displays for first time so players can see the board
	...
	while(!gamewon){//will stop when game is won, ! means NOT makes the oppisite be checked
		...
		if(charsPlaced == 42) break;//if draw
		hold2 = Insert(hold,player);//drop the player store the row in hold2
		if(hold2 == -1)	cout<<"Colomn is full\nPlease enter another number between 1 and 7:";//if error -1 row is full
		else{
			gamewon = Iswinning(hold2,hold);//check if game is run
			charsPlaced ++;//another character has been succesfully placed
			//This clears the screen works with windows, not nesscery to run game
            print();//displayed updated board
		}
	}
